/**
 * Batch 6 (ALL 20) outreach send — STRICTLY file-scoped to the 20 Batch-6
 * dealers in the ALL20 import CSV.
 *
 * This path can ONLY target the exact set of emails listed in the Batch 6
 ...
 * appears in this Batch 6 CSV — and even then the 30-day cooldown in
 ...
 *   - live send requires CONFIRM_DLR_BATCH6_SEND=SEND_20_DLR_BATCH6_PILOT_EMAILS
...
const REQUIRED_CONFIRM = 'SEND_20_DLR_BATCH6_PILOT_EMAILS'
const DEFAULT_CSV = 'outputs/utah_dealer_prospects_batch_6_ALL20_IMPORT.csv'
const ACTOR = { id: 'script:outreach-batch6-all20', email: 'brian@dlr-sms.com' }
...
async function main() {
  ...
  if (checkOnly) {
    ...
    console.log(`Batch 6 email scope (${emails.size}):`)
  ...
  if (requireSend && process.env.CONFIRM_DLR_BATCH6_SEND !== REQUIRED_CONFIRM) {
    console.error(`Refusing: set CONFIRM_DLR_BATCH6_SEND=${REQUIRED_CONFIRM} to allow the Batch 6 live send.`)
    process.exit(1)
  }
...
main().catch(err => {
  console.error('send-outreach-batch6-all20 failed:', err)
  process.exit(1)
})
