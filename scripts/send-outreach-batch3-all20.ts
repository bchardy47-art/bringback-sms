/**
 * Batch 3 (ALL 20) outreach send — STRICTLY file-scoped to the 20 Batch-3
 ...
 * in the Batch 3 ALL20 CSV. It loads that CSV, matches the emails against
 ...
 * in this Batch 3 CSV — and even then the 30-day cooldown in sendMonthlyInvite
 ...
 *   • live send requires CONFIRM_DLR_BATCH3_SEND=SEND_20_DLR_BATCH3_PILOT_EMAILS
...
const REQUIRED_CONFIRM = 'SEND_20_DLR_BATCH3_PILOT_EMAILS'
const DEFAULT_CSV = 'outputs/utah_county_dealer_prospects_batch_3_ALL20_IMPORT.csv'
const ACTOR = { id: 'script:outreach-batch3-all20', email: 'brian@dlr-sms.com' }
...
async function main() {
  ...
  if (checkOnly) {
    ...
    console.log(`Batch 3 email scope (${emails.size}):`)
  ...
  if (requireSend && process.env.CONFIRM_DLR_BATCH3_SEND !== REQUIRED_CONFIRM) {
    console.error(`Refusing: set CONFIRM_DLR_BATCH3_SEND=${REQUIRED_CONFIRM} to allow the Batch 3 live send.`)
    process.exit(1)
  }
  ...
  // ── Dry-run summary counts requested for Batch 3. ──
  ...
  console.log(
    `\nLIVE_SEND_COMMAND=OUTREACH_BUSINESS_ADDRESS="1346 W Fort Rock Dr, Saratoga Springs, UT 84045" ` +
      `CONFIRM_DLR_BATCH3_SEND=${REQUIRED_CONFIRM} OUTREACH_SEND_ENABLED=true ` +
      `npx tsx scripts/send-outreach-batch3-all20.ts --send`,
  )
}

main().catch(err => {
  console.error('send-outreach-batch3-all20 failed:', err)
  process.exit(1)
})
